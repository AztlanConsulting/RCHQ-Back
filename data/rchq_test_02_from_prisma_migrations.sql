-- Ejecutar con la sesión conectada a la base "RCHQ-test" (misma que DATABASE_URL en .env.test).
-- Ejecución: una sola vez en RCHQ-test vacía; si vuelves a correr el bloque CREATE obtendrás
-- "already exists". Alternativa sin SQL manual: dotenv -e .env.test -- npx prisma migrate deploy
-- (solo si aún no aplicaste nada; este script no escribe en _prisma_migrations, así que mezclar
-- a mano + migrate en la misma BD puede duplicar pasos: elige un camino o usa prisma db push
-- mientras desarrollas).
--
-- Contenido:
--   1) Copia fiel de prisma/migrations/20260419061249_init/migration.sql
--   2) Ajustes para alinear con prisma/schema.prisma (campos añadidos al modelo tras el init; aún
--      no hay un segundo directorio bajo prisma/migrations -- convierte esto a migración Prisma
--      con: npx prisma migrate dev, o usa: dotenv -e .env.test -- npx prisma migrate deploy
--
-- No reemplices esto con data/rchq_db.sql: ese archivo es un dump antiguo (distinto a Prisma).
--
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Alineación con prisma/schema.prisma (campos presentes en el modelo, ausentes
-- en 20260419061249_init). Idempotente: seguro re-ejecutar en BD ya parcial.
-- -----------------------------------------------------------------------------

ALTER TABLE "employee" ADD COLUMN IF NOT EXISTS "type" VARCHAR(20);
UPDATE "employee" SET "type" = 'internal' WHERE "type" IS NULL;
ALTER TABLE "employee" ALTER COLUMN "type" SET NOT NULL;

ALTER TABLE "employee" ADD COLUMN IF NOT EXISTS "end_date" DATE;
ALTER TABLE "employee" ADD COLUMN IF NOT EXISTS "phone_number" VARCHAR(20);
ALTER TABLE "employee" ADD COLUMN IF NOT EXISTS "salary" INTEGER NOT NULL DEFAULT 0;

ALTER TABLE "employee_address" ADD COLUMN IF NOT EXISTS "street" VARCHAR(200);
ALTER TABLE "employee_address" ADD COLUMN IF NOT EXISTS "municipio" VARCHAR(120);
ALTER TABLE "employee_address" ADD COLUMN IF NOT EXISTS "city" VARCHAR(100);
ALTER TABLE "employee_address" ADD COLUMN IF NOT EXISTS "postal_code" VARCHAR(10);

-- Ampliar "url" según @db.VarChar(200) en schema.prisma
