...
const buildWhere = (filters = {}) => {
  // deleted: "true" → eliminadas, cualquier otro valor → activas
  const isDeleted = filters.deleted === "true";
  const where = { is_deleted: isDeleted };

  // Construye employee de una sola vez para no sobreescribir
  const employeeFilter = {};
  if (filters.name)  employeeFilter.name  = { contains: filters.name,  mode: "insensitive" };
  if (filters.house) employeeFilter.house = { name: { contains: filters.house, mode: "insensitive" } };
