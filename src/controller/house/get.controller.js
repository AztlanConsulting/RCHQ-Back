...
exports.getHouseEmployees = async (req, res) => {
    try {
        const employeeId = req.user?.id;

        const result = await getHouseEmployeesForEmployee(employeeId);
        ...
        return res.status(500).json({
            success: false,
            message: "Respuesta inesperada al obtener empleados de la casa",
        });
    } catch (err) {
        console.error("getHouseEmployees error:", err);
        return res.status(500).json({
            success: false,
            message: "Error interno del servidor. Por favor intente más tarde.",
        });
